...
import { BarChart3, CircleDollarSign, Link2, Package, TrendingUp, Wallet, Zap } from 'lucide-react';
        ...
        <div className="grid gap-20 lg:grid-cols-[0.65fr_1.35fr] lg:items-start">
          ...
          <div className="sticky top-28">
            ...
            <motion.div
              ...
              className="mt-10 overflow-hidden rounded-2xl border border-(--landing-line) bg-(--landing-surface) p-6 shadow-sm"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-semibold text-(--landing-ink)">আপনার workflow-এর জন্য তৈরি</p>
                  <p className="mt-0.5 text-sm text-(--landing-muted)">আপনার content workflow-এর সাথে সহজেই মানিয়ে যায়।</p>
                </div>
              </div>

              <div className="mt-5 space-y-2">
                {['Dashboard access', 'Link tracking', 'Commission reports'].map((item, i) => (
                  <motion.div
                    key={item}
            ...
            {benefits.map((b, i) => {
              ...
              return (
                <motion.div
                  key={b.title}
                  variants={cardReveal}
                  whileHover={{ y: -6, boxShadow: '0 12px 40px -8px oklch(0.72 0.16 65 / 0.12)' }}
                  transition={{ type: 'spring', stiffness: 260, damping: 18 }}
                  className="group relative overflow-hidden rounded-2xl border border-(--landing-line) bg-(--landing-surface) p-7 transition-colors hover:border-(--landing-accent)/20"
                >
                  {/* Hover glow */}
                  <div className="pointer-events-none absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                    style={{ background: 'radial-gradient(circle at 50% 0%, oklch(0.72 0.16 65 / 0.06), transparent 60%)' }}
                  />
                  ...
                  <div className="relative">
                    <motion.div
                      ...
                      className="flex size-11 items-center justify-center rounded-2xl border border-(--landing-line) bg-(--landing-soft) shadow-sm transition-colors group-hover:border-(--landing-accent)/20 group-hover:bg-(--landing-accent)/5"
                    >
                      <Icon className="size-5 text-(--landing-accent)" />
                    </motion.div>

                    <h3 className="mt-5 font-semibold text-(--landing-ink)">{b.title}</h3>
                    <p className="mt-0.5 text-xs font-medium text-(--landing-accent)">{b.bangla}</p>
                    <p className="mt-2.5 text-sm leading-6 text-(--landing-muted)">{b.description}</p>
                  </div>
                </motion.div>
              );
            })}
